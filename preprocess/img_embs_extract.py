...
if __name__ == "__main__":
    ...
    img_embs_save_path = DATA_PATH / "preprocessed" / "img_embs.pt"
    img_embs_dict = {}
    for img_file in tqdm(glob(str(DATA_PATH / "preprocessed" / "images" / "*.jpg"))):
        inter_id = int(Path(img_file).stem)
        image = Image.open(img_file).convert('RGB')
        inputs = processor(images=image, return_tensors="pt")
        pixel_values = inputs['pixel_values']  # (1, 3, 224, 224)
        with torch.no_grad():
            img_embs = model(pixel_values=pixel_values).last_hidden_states  # (1, seq_len, hidden_size) [1, 197, 768]
